...
void CTriangle::Render(sf::RenderWindow& window)
{
	if (m_selected)
	{
		sf::FloatRect globalBounds = m_triangle.getGlobalBounds();
		...
		boundingBox.setOutlineThickness(TRIANGLE_OUTLINE_THICKNESS);
...
void CTriangle::SetFigureColor(const sf::Color& fill)
{
	m_triangle.setFillColor(fill);
}

void CTriangle::SetOutlineThickness(float thickness)
{
	m_triangle.setOutlineThickness(thickness);
}

void CTriangle::SetOutlineColor(const sf::Color& fill)
{
	m_triangle.setOutlineColor(fill);
}